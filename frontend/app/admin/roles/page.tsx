...
async function fetchData<T>(path: string): Promise<T> {
  ...
  if (!res.ok) throw new Error(await responseDetail(res, "No se pudo cargar la información."));
  return res.json();
}

async function saveData<T>(path: string, method: "POST" | "PATCH", body: unknown): Promise<T> {
  const res = await fetchWithAuth(`${API_BASE}${path}`, {
    method,
    body: JSON.stringify(body),
  });
  if (!res.ok) throw new Error(await responseDetail(res, "No se pudo guardar el rol."));
  return res.json();
}
...
function statusLabel(status: Capability["status"]) {
  ...
  if (status === "available") return "Transición";
  return "Reservado";
}
...
function CapabilityMatrix({
  ...
  const groups = useMemo(() => {
    ...
    for (const cap of catalog.capabilities) {
      const current = map.get(cap.module) ?? [];
      current.push(cap);
      map.set(cap.module, current);
    }
  ...
  return (
    <div className="space-y-4">
      {groups.map(([module, capabilities]) => (
        <section key={module} className="rounded-xl border border-white/[0.06] bg-black/20 p-4">
          ...
          <div className="grid gap-2 lg:grid-cols-2">
            {capabilities.map((cap) => {
              const checked = selected.includes(cap.code);
              const mayDelegate = catalog.is_platform_admin || held.has(cap.code);
              const locked = disabled || !cap.assignable || !mayDelegate;
              return (
                <label
                  key={cap.code}
                  className={`flex gap-3 rounded-lg border px-3 py-3 ${
                    checked ? "border-white/20 bg-white/[0.04]" : "border-white/[0.05]"
                  } ${locked ? "opacity-55" : "cursor-pointer"}`}
                  title={!cap.assignable ? "Este módulo todavía no puede asignarse." : !mayDelegate ? "No puedes delegar un permiso que tú mismo no tienes." : undefined}
                >
                  <input
                    type="checkbox"
                    checked={checked}
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="mt-1 block text-xs leading-5 text-zinc-600">{cap.description}</span>
                    <code className="mt-1.5 block text-[10px] text-zinc-700">{cap.code}</code>
                  </span>
                </label>
              );
            })}
...
function RoleEditor({
  ...
  const held = new Set(catalog.held_by_me);
  const containsAuthorityICannotDelegate = !catalog.is_platform_admin && role.capabilities.some((code) => !held.has(code));
  const editable = canManage && !containsAuthorityICannotDelegate;
  ...
  async function save() {
    ...
    setError(null);
    try {
      await saveData(`/admin/roles/${role.id}/`, "PATCH", {
        name: name.trim(),
    ...
    <article className="rounded-xl border border-white/[0.07] bg-white/[0.02]">
      <button type="button" onClick={() => setOpen((value) => !value)} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full px-2 py-0.5 text-[10px] ${role.is_active ? "bg-white/[0.06] text-zinc-300" : "bg-red-500/10 text-red-400"}`}>
      ...
      {open ? (
        <div className="border-t border-white/[0.06] px-5 py-5">
          {containsAuthorityICannotDelegate ? (
            <div className="mb-4 rounded-lg border border-amber-500/20 bg-amber-500/[0.05] px-4 py-3 text-xs text-amber-200/75">
              Este rol contiene permisos que tu cuenta no posee. Puedes revisarlo, pero no editarlo: el backend impide delegar o conservar autoridad superior a la propia.
            </div>
          ) : null}

          <div className="mb-5 grid gap-4 md:grid-cols-[1fr_2fr_auto]">
            <label className="text-xs text-zinc-500">
              Nombre
              <input value={name} onChange={(event) => setName(event.target.value)} disabled={!editable || busy} className="mt-1.5 w-full rounded-lg border border-white/[0.08] bg-black/40 px-3 py-2.5 text-sm text-zinc-200" />
            </label>
            <label className="text-xs text-zinc-500">
              Descripción
              <input value={description} onChange={(event) => setDescription(event.target.value)} disabled={!editable || busy} className="mt-1.5 w-full rounded-lg border border-white/[0.08] bg-black/40 px-3 py-2.5 text-sm text-zinc-200" />
            </label>
            <label className="flex items-end gap-2 pb-2 text-xs text-zinc-400">
              <input type="checkbox" checked={active} onChange={(event) => setActive(event.target.checked)} disabled={!editable || busy} />
              Rol activo
            </label>
          </div>
          ...
          {editable ? (
            <div className="mt-5 flex justify-end">
              <button type="button" disabled={busy || !name.trim()} onClick={() => void save()} className="rounded-lg bg-white px-5 py-2.5 text-sm font-semibold text-black hover:bg-zinc-200 disabled:opacity-40">
                Guardar rol
              </button>
            </div>
          ) : null}
...
function CreateRole({
  ...
  async function create() {
    ...
    setError(null);
    try {
      await saveData("/admin/roles/", "POST", {
        company: companyId,
        name: name.trim(),
  ...
  if (!open) {
    return <button type="button" onClick={() => setOpen(true)} className="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-black hover:bg-zinc-200">Nuevo rol</button>;
  }
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <h2 className="font-semibold text-white">Nuevo rol empresarial</h2>
          <p className="mt-1 text-xs text-zinc-600">Parte del mínimo necesario; agrega solo la autoridad que el trabajo requiere.</p>
        </div>
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [roleData, catalogData] = await Promise.all([
        fetchData<{ results: Role[] }>(`/admin/roles/${qs}`),
        fetchData<CapabilityCatalog>(`/admin/capabilities/${qs}`),
      ]);
      ...
      <div className="space-y-7">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-zinc-500">Configura autoridad por empresa con principio de mínimo privilegio. El backend nunca permite que un administrador delegue permisos que no posee.</p>
          </div>
          <div className="flex gap-2">
            ...
            {canManage && companyId && catalog ? <CreateRole companyId={companyId} catalog={catalog} onCreated={load} /> : null}
          </div>
        </div>

        {!companyId ? <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.05] p-5 text-sm text-amber-200/80">Selecciona una empresa para revisar sus roles. El master de plataforma tiene autoridad global, pero nunca edita un tenant implícito.</div> : null}
        ...
        {companyId && catalog && !loading && !error ? (
          <>
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-xs text-zinc-500">
              <strong className="text-zinc-300">Importante:</strong> “Activo” significa que el permiso ya gobierna endpoints actuales. “Transición” identifica módulos que todavía conservan compatibilidad legacy. “Reservado” describe funcionalidad futura y no puede asignarse.
            </div>

            <div className="space-y-3">
              {roles.map((role) => <RoleEditor key={role.id} role={role} catalog={catalog} canManage={canManage} onSaved={load} />)}
