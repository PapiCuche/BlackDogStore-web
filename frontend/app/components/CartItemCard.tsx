    ...
    <div className="rounded-2xl border border-white/[0.08] bg-surface p-5 transition hover:border-white/15">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="font-display font-black uppercase text-white">{product.name}</p>
            <p className="text-sm text-zinc-600">S/ {formatMoney(product.price)} c/u</p>
          </div>
        </div>

        <div className="flex items-center justify-between gap-4 sm:justify-end">
          <div className="flex items-center gap-2">
            <label className="text-xs uppercase tracking-widest text-zinc-600">Cant.</label>
            <input
              type="number"
              min={1}
              value={quantity}
              onChange={(e) => onQuantityChange?.(Number(e.target.value))}
              className="w-16 rounded-xl border border-white/[0.08] bg-white/[0.04] px-2 py-1.5 text-center text-sm text-white focus:border-white/25 focus:outline-none"
            />
          </div>
          <p className="font-display font-black text-white">S/ {formatMoney(Number(product.price) * quantity)}</p>
          {onRemove && (
            <button
              type="button"
              onClick={onRemove}
